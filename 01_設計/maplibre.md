# レイヤーの前面・背面表示の切り替えは、map.moveLayer メソッドを使用してレイヤーの順序を変更することで実現できます。以下の例では、ボタンをクリックすることでレイヤーの順序を変更する方法を示します。

## この修正では、moveLayerToFront 関数と moveLayerToBack 関数を追加し、ボタンをクリックすることでレイヤーの順序を変更できるようにしています。moveLayerToFront 関数は指定したレイヤーを最前面に移動し、moveLayerToBack 関数は指定したレイヤーを最背面に移動します。
...
const MapApp: React.FC = () => {
  ...
  const layers: Layer[] = [
    {
      ...
      source: {
        type: "raster",
        tiles: [
          "https://ktgis.net/kjmapw/kjtilemap/tokyo50/03/{z}/{x}/{y}.png",
        ],
        tileSize: 512,
        scheme: "tms",
        maxzoom: 16,
  ...
  const moveLayerToFront = (layerId: string) => {
    if (mapInstance.current) {
      mapInstance.current.moveLayer(layerId);
    }
  };

  const moveLayerToBack = (layerId: string) => {
    if (mapInstance.current) {
      const layers = mapInstance.current.getStyle().layers;
      if (layers && layers.length > 0) {
        mapInstance.current.moveLayer(layerId, layers[0].id);
      }
    }
  };
  ...
  return (
    <div className="absolute inset-0 z-0">
      ...
      <button onClick={() => moveLayerToFront("polygon")}>
        Move Polygon to Front
      </button>
      <button onClick={() => moveLayerToBack("polygon")}>
        Move Polygon to Back
      </button>
    </div>
  );
};
